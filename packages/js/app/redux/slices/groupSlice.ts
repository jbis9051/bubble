...
export const authSlice = createSlice({
    name: 'group',
    initialState: groupsAdapter.getInitialState<{ status: GroupStateStatus, activeGroupUuid: uuid }>({
        status: 'idle',
        activeGroupUuid: "",
    }),
    reducers: {
...
const groupsSelector = groupsAdapter.getSelectors<RootState>(state => state.groups);

export const selectGroups = groupsSelector.selectAll;
export const selectGroupById = (state: RootState, id: uuid) => groupsSelector.selectById(state, id);
export const selectCurrentGroup = (state: RootState) => groupsSelector.selectById(state, state.groups.activeGroupUuid);
