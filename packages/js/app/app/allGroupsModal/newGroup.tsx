...
export default function NewGroup() {
    ...
    const handleCreate = () => {
        if (!name.length) return Alert.alert("Please enter a name for your Bubble");
        setLoading(true);
        GroupService
            .create_group(name)
            .then((newGroupUuid) => {
    ...
    if (createdUuid) {
        return (
            <View style={styles.container}>
                <StyledText variant="h2" nomargin style={{ marginBottom: 15, }}>Bubble Created!</StyledText>
    ...
    return (
        <View style={styles.container}>
            <StyledText variant="h2" nomargin style={{ marginBottom: 15, }}>Name your Bubble</StyledText>
            <StyledInput
                label="Bubble Name"
                value={name}
                onChange={setName}
